...
- Structured routing pipeline: routes → handlers → services → external clients
...
│   ├── server.ts              # Server bootstrap with dependency graph
...
- `npm test` – run all Vitest suites (unit, integration, e2e)
...
- ✅ Fastify server with modular routes/handlers/services
- ✅ OpenAI + Chroma retrieval-assisted WhatsApp responses
- ✅ Deterministic fakes for integration and e2e tests
- ✅ Google Drive CSV export for conversation transcripts
- ✅ Zod runtime environment validation
- 🔄 Continuing work on prompt iteration, analytics, and observability
